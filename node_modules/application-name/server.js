...
const key = "WWOvFtstHB73Ao9lNvAwOcyN7Tf//glDdlVunWNJs19hirJfM1zmD0zdGfsuHd4d1u99TT//njEnhJksbMVLYw==" , storage = "portalvhdsq1p0ksz90hvrb" , container = "pictures" ;
...
function iniciaServico(){
	...
	var servico = azure.createBlobService(storage , key);
	//cria pasta se não existir na API 
	servico.createContainerIfNotExists(container, function (error) {
	  if (error) {
	    console.log("Erro ao criar pasta Azure");
	  } else { 
...
app.get("/listar" , function(req , res){	
	...
	servico.listBlobs(container , function(err , files){
		res.render("listar" ,{
			files : files
		});
	});
...
app.get("/visualizar/:id" , function(req , res){
	...
	servico.getBlobToStream(container , id , res , function(err){
		if(err){
...
app.get("/deletar/:id" , function(req , res){
	...
	//iniciando conexao com a API do Azure Storage
	var servico = iniciaServico();
	servico.deleteBlob(container , id , function(err){
		if(err){
